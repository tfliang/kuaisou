...
class KsFeature:
    ...
    # 注意防止信息穿越
        ...
        for uid, data in all_data.items():
            source = data['source']
            device = min(data['device'], 40)
            reg_day = data['reg_day'] % 7
    ...
    def pre_set_single(self, all_data, min_log):
        if len(self.uf) >= len(all_data):
            uf = self.uf
        else:
            uf = self.user_feature(all_data)
    ...
    def submit_uid(self, dataB, dataA, min_log, num_pre, num_tree=1000, fname='uids_b_r2.csv', ntop=40000, repeat=True):
        ...
        #pd.DataFrame([tmp[0] for tmp in pre]).to_csv(fname, header=False, index=False)
    ...
    def single_model(self, dataB, dataA, num_pre, num_tree=100, split=True, repeat=False):
        all_score = []
        all_pre = {}
        all_clf = []
        dataB = self.filter(dataB)
        dataA = self.filter(dataA)
        for min_log in range(1, 31-num_pre):
            clf, score = self.train_model(dataB, dataA, min_log, num_pre, num_tree, split=split, repeat=repeat)
            df = self.pre_set_single(dataB, min_log)
            feature_name = list(clf.feature_name())
            pre_df = clf.predict(df[feature_name])
            pre = [(int(df.iloc[i]['uid']), pre_df[i]) for i in range(0, len(pre_df))]
            pre.sort(key=lambda x:x[1], reverse=True)
            mm = len([tmp for tmp in pre if tmp[1] >= 0.5])
            all_pre[min_log] = pre
            tmp = [min_log, score, clf.best_iteration, score[-2]/score[-1]*mm, mm]
            all_score.append(tmp)
            all_clf.append(clf)
        for min_log in range(31-num_pre, 31):
            df = self.pre_set_single(dataB, min_log)
            pre = [[int(df.iloc[i]['uid'])] for i in range(0, len(df))]
            for clf in all_clf:
                pre_df = clf.predict(df[feature_name])
                for i in range(0, len(pre_df)):
                    pre[i].append(pre_df[i])
            pre.sort(key=lambda x:x[-1], reverse=True)
            all_pre[min_log] = pre
            mm = len([tmp for tmp in pre if tmp[-1] >= 0.5])
            all_score.append([mm*0.8,mm])
        sp = 0
        sm = 0
        for tmp in all_score:
            print('--------------', min_log)
            print(tmp)
            sp += tmp[-2] 
            sm += tmp[-1]
        F_score = 2*sp / (sm + 23722)
        self.all_score = all_score
        self.all_pre = all_pre
        self.all_clf = all_clf
        print(F_score)
        joblib.dump(all_pre, 'all_pre_limit.pkl')
