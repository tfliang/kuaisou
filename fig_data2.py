...
f_data = './data/2_pre_data.json'
f_pk = './data/2_pre_data.pkl'
f_pk_d1 = './data/2_pre_data_d1.pkl'

def load_data():
    #rs = pk.load(open(f_pk,'rb'))
    rs = pk.load(open(f_pk_d1,'rb'))
    '''
    rs = {}
...
# fig life of user
def fig_life_group_device(rs, day, n):
    # filter
    rs_filter = {}
    for uid, data in rs.items():
        #if data['userdata'][0]==24 and data['userdata'][1]==3:
        if data['userdata'][0]==day and data['userdata'][2]==1:
            rs_filter[uid] = data
    S = len(rs_filter)
    ...
    '''
    uids_d = [0 for i in range(0,day-1)]
    for i in range(day,31-n):
        after_uids = set()
        for s in range(1,n+1):
            after_uids = after_uids | uids[i+s]
        uids_d.append(len(uids[i] | after_uids))
    uids_d.extend([0 for i in range(0,n)])
    tmp = [uids_d[i]/uids_d[day-1] for i in range(0,len(uids_d))]
    rate_tmp = tmp[day-1:]
    mydata.append(go.Bar(x=x,y=tmp,name='after'))
    #mydata.append(go.Bar(x=x,y=uids_d,name='after'))
    '''
...
def gether_rate(rs):
    all_rate = {}
    for i in range(1,23):
        rate = fig_life_group(rs,i,6)
        for dd, rr in enumerate(rate):
            if rr > 0.1:
                all_rate.setdefault(dd,[]).append(rr)
    kk = list(all_rate.keys())
    kk.sort()
    avg_rate = []
    for k in kk:
        avg_rate.append(sum(all_rate[k])/len(all_rate[k]))
    return (avg_rate, all_rate)

def pre_amount(rs):
    rate,all_rate = gether_rate(rs)
    print(rate)
    num_u = {}
    pre_num = {}
    for uid, data in rs.items():
        day = data['userdata'][0]
        num_u.setdefault(day,0)
        num_u[day] += 1
    for i in range(1,31):
        days = 31-i
        if days >= len(rate):
            ...
            rr = rate[days]
        pre_num[i] = (num_u[i], rr, int(rr*num_u[i]))
        #print(rr,num_u[i],rr*num_u[i])
    pk.dump(pre_num, open('./data/pre_num.pkl','wb'))
